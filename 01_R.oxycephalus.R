...
presencedata <- st_read("./data_raw/R.oxycephalus_occ.gpkg", quiet = FALSE) %>%
...
if(nrow(bgdata2)<6562) # Use all segments if fewer than target sample size
{
  samplesize = nrow(bgdata2)
} else {
  samplesize = 6562 # Adjustable sampling size depending on study design
}
...
mod_maxent <- maxent(
  ...
  path = "./data_outcome/R.oxycephalus_maxent_result"
)

# Save trained MaxEnt model as .rds
saveRDS(mod_maxent, file = "./data_outcome/R.oxycephalus_maxent_result.rds")

# Load saved model
mod_maxent <- readRDS("./data_outcome/R.oxycephalus_maxent_result.rds")
...
# Scenario-specific climate variables (BIO05, BIO12, BIO)
...
binary_future <- bind_rows(
  make_binary_long(streamnetwork_pred_s1_21_40_binary, "R.oxycephalus", "SSP126", "2021-2040"),
  make_binary_long(streamnetwork_pred_s1_41_60_binary, "R.oxycephalus", "SSP126", "2041-2060"),
  make_binary_long(streamnetwork_pred_s1_61_80_binary, "R.oxycephalus", "SSP126", "2061-2080"),
  make_binary_long(streamnetwork_pred_s1_81_100_binary, "R.oxycephalus", "SSP126", "2081-2100"),
  
  make_binary_long(streamnetwork_pred_s2_21_40_binary, "R.oxycephalus", "SSP245", "2021-2040"),
  make_binary_long(streamnetwork_pred_s2_41_60_binary, "R.oxycephalus", "SSP245", "2041-2060"),
  make_binary_long(streamnetwork_pred_s2_61_80_binary, "R.oxycephalus", "SSP245", "2061-2080"),
  make_binary_long(streamnetwork_pred_s2_81_100_binary, "R.oxycephalus", "SSP245", "2081-2100"),
  
  make_binary_long(streamnetwork_pred_s5_21_40_binary, "R.oxycephalus", "SSP585", "2021-2040"),
  make_binary_long(streamnetwork_pred_s5_41_60_binary, "R.oxycephalus", "SSP585", "2041-2060"),
  make_binary_long(streamnetwork_pred_s5_61_80_binary, "R.oxycephalus", "SSP585", "2061-2080"),
  make_binary_long(streamnetwork_pred_s5_81_100_binary, "R.oxycephalus", "SSP585", "2081-2100")
)

# Combine present + future 
R_oxycephalus_binary_suitability_long_result <- bind_rows(
  binary_present,
  binary_future
)

# Check result
R_oxycephalus_binary_suitability_long_result %>%
  count(ssp_scenario, time, occurrence, drop = FALSE)

# Export CSV
write.csv(
  R_oxycephalus_binary_suitability_long_result,
  "./data_outcome/R.oxycephalus_binary_suitability_long_result.csv",
  row.names = FALSE,
  na = "NA"
)
...
results_csv <- read.csv(
  "./data_outcome/R.oxycephalus_maxent_result/maxentResults.csv"
)
